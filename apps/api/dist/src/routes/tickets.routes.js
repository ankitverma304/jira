...
import { z } from "zod";
import { BUG_SEVERITIES, HISTORY_TYPES, POINT_EVENT_TYPES, ROLES, TICKET_PRIORITIES, TICKET_STATUSES, TICKET_WORKFLOW } from "../constants.js";
...
function getBugPointDelta(severity) {
    switch (severity) {
        case "MINOR":
            return -1;
        case "MAJOR":
...
function getBugEventType(severity) {
    switch (severity) {
        case "MINOR":
...
function canTransitionStatus(currentStatus, nextStatus, actorRole, actorId, assigneeId) {
...
router.post("/", requireRole([...ROLES]), async (req, res) => {
    ...
    const notificationTargets = [assigneeId, ...body.watcherIds].filter((userId) => Boolean(userId) && userId !== req.user.id);
...
router.put("/:id", requireRole([...ROLES]), async (req, res) => {
    ...
    if (body.status && body.status !== existing.status) {
        ...
        const recipients = [ticket.assigneeId, ...fromDelimitedString(ticket.watchers)].filter((userId) => Boolean(userId) && userId !== req.user.id);
