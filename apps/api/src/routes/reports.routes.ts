...
router.get("/overview", async (_req, res) => {
  ...
  const projectCompletion = projects.map((project) => {
    ...
    const overdue = project.tickets.filter(
      (ticket) => ticket.endDate && !["RESOLVED", "CLOSED"].includes(ticket.status) && ticket.endDate < new Date()
    ).length;
  ...
  const pointsTrendMap = points.reduce<Record<string, number>>((acc, entry) => {
...
router.get("/project-wise", async (_req, res) => {
  ...
  res.json(
    projects.map((project) => ({
      ...
      overdueTickets: project.tickets.filter(
        (ticket) => ticket.endDate && !["RESOLVED", "CLOSED"].includes(ticket.status) && ticket.endDate < new Date()
      ).length
    }))
  );
...
router.get("/bug-analytics", async (_req, res) => {
  ...
  const bySeverity = bugs.reduce<Record<string, number>>((acc, bug) => {
    acc[bug.severity] = (acc[bug.severity] ?? 0) + 1;
    return acc;
  }, {});

  const trend = bugs.reduce<Record<string, number>>((acc, bug) => {
...
router.get("/timeline/:userId", async (req, res) => {
  ...
  const dailyHours = logs.reduce<Record<string, number>>((acc, log) => {
    const key = dayjs(log.workDate ?? log.loggedAt).format("YYYY-MM-DD");
    acc[key] = (acc[key] ?? 0) + log.hours;
    return acc;
  }, {});
  ...
  res.json({
    ...
    bugs: pointEvents.reduce<Record<string, number>>((acc, entry) => {
